...
const FoldText = ({
  ...
  useEffect(() => {
    ...
    if (trigger === 'hover') {
      ...
      gsap.set(pieces, fromVars);
      const scrollExpand = root.closest('.scroll-expand');
      if (!scrollExpand) return undefined;
      const onScrollExpandEnd = () => play(false);
      scrollExpand.addEventListener('scroll-expand-end', onScrollExpandEnd, { once: true });
      scrollTrigger = { kill: () => scrollExpand.removeEventListener('scroll-expand-end', onScrollExpandEnd) };
